...
async function resolveFont(font: string, fontUrl?: string | null) {
...
class GalleryApp {
  ...
  onClick(e: MouseEvent) {
    ...
    for (const media of this.medias) {
      ...
      if (worldX >= posX - halfWidth && worldX <= posX + halfWidth) {
        const originalIndex = media.index % this.originalLength
        this.onItemActivate?.(originalIndex)
        // Snap à l'item cliqué
        this.goToIndex(originalIndex)
        break
      }
    }
  }
  ...
  goToIndex(index: number) {
    ...
    // Trouve le multiple de widthTotal le plus proche
  ...
  update() {
    ...
    if (this.medias) {
      ...
      // Détection de l'item actif (plus proche du centre)
      let activeIndex = 0
...
const CircularGallery = forwardRef<CircularGalleryRef, CircularGalleryProps>(
  function CircularGallery(
    ...
    // Résolution asynchrone de la police
    useEffect(() => {
      let cancelled = false
      resolveFont(font, fontUrl).then((f) => {
        if (!cancelled) setResolvedFont(f)
      })
      return () => {
        cancelled = true
      }
    ...
    // Initialisation / destruction de la scène WebGL
    useEffect(() => {
      ...
      const init = () => {
        app = new GalleryApp(containerRef.current!, {
          items: items.map((it) => ({ image: it.image, text: it.text || it.name || '' })),
          bend,
          textColor,
          borderRadius,
          font: resolvedFont,
    ...
    }, [items, bend, textColor, borderRadius, resolvedFont, scrollSpeed, scrollEase, onActiveIndexChange, onItemActivate])

    // Exposition next / prev
    useImperativeHandle(
      ref,
      () => ({
    ...
    return (
      <div
        ...
        tabIndex={0}
        role="region"
        aria-label="Galerie circulaire — flèches gauche/droite, molette ou glisser pour naviguer. Cliquez sur une image pour la sélectionner."
      />
    )
  }
)
